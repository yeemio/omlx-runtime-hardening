...
class EnginePool:
    ...
    def __init__(
        ...
        self._restart_requested: bool = False  # swap-safe: set when memory settle fails
        self._restart_reason: str = ""  # swap-safe: reason for restart request
        self._last_eviction: dict | None = None  # swap-safe P3: last eviction event
    ...
    def last_eviction(self) -> dict | None:
        """Last eviction event details (swap-safe P3)."""
        return self._last_eviction

    def get_loaded_model_details(self) -> list[dict]:
        """Get details of all currently loaded models (swap-safe P3)."""
        now = time.time()
        details = []
    ...
    async def get_engine(
        ...
        async with self._lock:
            ...
            # swap-safe: Pre-load watermark check with hot-cache LRU eviction
            if self._process_memory_enforcer is not None:
                enforcer = self._process_memory_enforcer
                if hasattr(enforcer, 'pre_load_check'):
                    ...
                    if action.value in (
                        ...
                        # P3: Log eviction candidates before starting
                        candidates_info = []
                        now = time.time()
                        for mid, e in self._entries.items():
                        ...
                        logger.info(
                            f"[swap-safe] Eviction candidates: {candidates_info}"
                        )
                        ...
                        while True:
                            ...
                            logger.info(
                                f"[swap-safe] After evicting '{victim}': "
                                f"watermark={new_wm} "
                                f"projected={diagnostics.get('projected_gb', '?')}GB"
                            )
                            if action.value == "load_directly":
                                break
                            # Yellow is tolerable — one more reclaim pass will handle it
                            if action.value == "reclaim_then_load":
                                break

                    # Phase 2: If still elevated, lightweight emergency reclaim
                    if action.value in (
                        "reclaim_then_load", "restart_then_load", "queue_and_wait",
                    ):
                    ...
                    # Phase 3: Only flag restart if still red/fatal after all efforts
                    ...
                    if action.value in ("restart_then_load", "queue_and_wait"):
                        ...
                        logger.error(
                            f"[swap-safe] RESTART RECOMMENDED before loading {model_id}"
                        )

                    logger.info(
                        f"[swap-safe] Pre-load summary: "
                        f"watermark_before={watermark_before} "
                        f"watermark_after={watermark_after} "
                        f"evicted_models={evicted_models} "
                        f"freed_model_est_gb={freed_est_gb} "
    ...
    def _find_lru_victim(self, exclude: set[str] | None = None) -> str | None:
        ...
        swap-safe P0: Skips models with active inference requests to avoid
        ...
        for mid, e in self._entries.items():
            ...
            if exclude and mid in exclude:
                continue
            # P0: Skip models with active inference requests
            try:
                if e.engine.has_active_requests():
                    logger.debug(
                        f"[swap-safe] Skipping victim '{mid}': has active requests"
                    )
                    continue
            except AttributeError:
                pass  # Engine type doesn't expose has_active_requests
    ...
    async def _unload_engine(self, model_id: str) -> None:
        ...
        swap-safe: After stopping the engine, polls mx.get_active_memory()
        to verify Metal buffers are actually reclaimed before proceeding.
        ...
        # swap-safe P1: Memory settle barrier based on real freed delta.
        # Compare pre/post unload active memory instead of relying on
        # the cumulative _current_model_memory estimate.
        ...
        for _settle_round in range(10):
            ...
            if actual_freed >= min_expected_freed:
                settled = True
                logger.debug(
                    f"[swap-safe] Settle round {_settle_round + 1} for '{model_id}': "
        ...
        # Release memory tracking AFTER barrier (not before)
        ...
        if settled:
            ...
            logger.warning(
                f"[swap-safe] Settle barrier timed out for '{model_id}': "
                f"freed={format_size(actual_freed)} "
