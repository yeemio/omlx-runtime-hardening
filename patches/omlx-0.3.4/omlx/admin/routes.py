...
async def get_restart_status(is_admin: bool = Depends(require_admin)):
    """Get engine restart status and memory diagnostics (swap-safe)."""
    from datetime import datetime, timezone
    engine_pool = _get_engine_pool()
    if engine_pool is None:
        raise HTTPException(status_code=503, detail="Server not initialized")
    ...
    # P3d: Compute watermark from current utilization
...
async def restart_engine(is_admin: bool = Depends(require_admin)):
    """Request or clear engine restart flag (swap-safe)."""
    engine_pool = _get_engine_pool()
    if engine_pool is None:
        raise HTTPException(status_code=503, detail="Server not initialized")
